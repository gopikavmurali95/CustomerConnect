...
@injectable
class CustomerFocHeaderBloc extends Bloc<CustomerFocHeaderEvent, CustomerFocHeaderState> {
  final ICustomerFocRepo headerRepo;
  CustomerFocHeaderBloc(this.headerRepo) : super(CustomerFocHeaderState.initial()) {
    on<GetCustomerFocHeaderEvent>((event, emit) async{
      List<CustomerFocHeaderModel> searcheditems = [];
      Either<MainFailures, List<CustomerFocHeaderModel>> data = await headerRepo.getHeaderList(event.statusValue);
      emit(data.fold((l) => const CustomerFocHeaderFailedState(),
       (r) {
        searcheditems = r
                .where((element) =>
                    element.cfhRtnId!
